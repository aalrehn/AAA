...
void findLargestRedCandle(const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], int rates_total, int previousCandlesToCompare)
{
   ...
   {
      if (close[i] < open[i])  // Change condition to detect red candles
      {
         ...
         if (isLargerThanPrevious && ((currentCandleLow - close[i]) <= (candleLength * 0.15))) // Adjusted to ensure low is close to close price
         {
   ...
   Print("No qualifying red candle found.");
}
